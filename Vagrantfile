...
php_xdebug            = "true"   # Only applies to php install (not hhvm)
...
lumen_root_folder     = "/vagrant/lumen" # Where to install Lumen. Will `composer install` if a composer.json file exists
lumen_version         = "lastest-stable" # If you need a specific version of Lumen, set it here
...
Vagrant.configure("2") do |config|
  ...
  # Provision Lumen
  # config.vm.provision "shell", path: "#{github_url}/scripts/lumen.sh", privileged: false, args: [server_ip, lumen_root_folder, public_folder, lumen_version]
